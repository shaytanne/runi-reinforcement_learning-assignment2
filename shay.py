...
class BaseAgent(ABC):
    ...
    """
    def __init__(self, num_states: int, num_actions: int, lr: float = 0.1, gamma: float = 0.99, epsilon: float = 0.1, q_init: float | None = 0.0):
...
def key_door_reward_shaping_func(env: KeyFlatObsWrapper, reward: float , key_bonus_given: bool, door_bonus_given: bool, agent) -> float:
    ...
    """
    is_mc = isinstance(agent, MCAgent)
    if is_mc:
      step_penalty = 0.0
      key_bonus = 25.0
      door_bonus = 0.0
    ...
    def _run(self, num_episodes: int, training: bool, use_shaping: bool, force_greedy: bool, return_diagnoastics: bool = False
        ...
        for episode in range(num_episodes):
            ...
            if training and hasattr(self.agent, "epsilon"):
                if isinstance(self.agent, SARSAAgent) and self.agent.epsilon > 0.05:
                    self.agent.epsilon *= 0.997
                else:
